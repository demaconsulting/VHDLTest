...
xUnit (packages `xunit.v3` and `xunit.runner.visualstudio`) is the unit-testing framework
used by VHDLTest. It provides test discovery, execution, and result reporting for all test
projects. xUnit is not deployed with VHDLTest; it is a build-time and CI-only dependency
included as a `PrivateAssets` reference in the test project.

### Features Used

- **Test discovery**: the `[Fact]` and `[Theory]` attributes mark test methods; xUnit
...
xUnit is referenced in `test/DEMAConsulting.VHDLTest.Tests/DEMAConsulting.VHDLTest.Tests.csproj`
