...
- **Good**: `VHDLTest-Core-DisplayHelp`
...
Platform-specific and simulator-specific requirements MUST use source filters
...
tests:
  ...
  - "macos@TestMethodName"      # macOS platform evidence only
  - "ghdl@TestMethodName"       # GHDL simulator evidence only
  - "nvc@TestMethodName"        # NVC simulator evidence only
...
- [ ] Simulator-specific requirements use source filters (`ghdl@TestName`, `nvc@TestName`)
...
- [ ] ReqStream enforcement passes: `dotnet reqstream --requirements requirements.yaml --tests "artifacts/**/*.trx" --enforce`
